import { z } from 'zod';
export const readingEntrySchema = z.object({ word: z.string().min(1).max(100), reading: z.string().min(1).max(200) });
...
export function applyReadings(text: string, entries: ReadingEntry[]) {
  ...
  const pattern = new RegExp(sorted.map((entry) => entry.word.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|'), 'g');
