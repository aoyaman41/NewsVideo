...
export function measurePcmWav(bytes: Uint8Array): AudioMeasurement {
  ...
  if (bytes.length < 44 || tag(0) !== 'RIFF' || tag(8) !== 'WAVE')
    throw new Error('PCM WAV形式の音声が必要です。');
  let format = 0,
    channels = 0,
    sampleRate = 0,
    bits = 0,
  ...
  if (format !== 1 || bits !== 16 || !channels || !sampleRate || !length)
    throw new Error('16ビットPCM WAVの実測に対応しています。');
  let peak = 0,
    squares = 0,
    silence = 0,
    clipped = 0;
  ...
  for (let index = 0; index < count; index++) {
    const value = Math.abs(view.getInt16(start + index * 2, true)) / 32768;
    peak = Math.max(peak, value);
    squares += value * value;
    if (value < 0.003162) silence++;
    if (value >= 0.999) clipped++;
  }
  return {
    durationSec: count / channels / sampleRate,
    peakDbfs: 20 * Math.log10(Math.max(peak, 1e-8)),
    rmsDbfs: 20 * Math.log10(Math.max(Math.sqrt(squares / count), 1e-8)),
