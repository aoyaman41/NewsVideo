...
it('distinguishes estimated timing and exports precise SRT and VTT timestamps', () => {
  const cues = estimateCaptions(createNewPart(0, { scriptText: '最初の文。次の文。', durationEstimateSec: 10 }));
...
it('applies the longest matching dictionary entry without re-replacing generated readings', () => {
  expect(applyReadings('OpenAI API', [{ word: 'OpenAI', reading: 'オープンエーアイ' }, { word: 'OpenAI API', reading: '専用の読み' }, { word: '専用', reading: '変更' }])).toBe('専用の読み');
  expect(applyReadings('C++', [{ word: 'C++', reading: 'シープラスプラス' }])).toBe('シープラスプラス');
});
it('measures PCM duration, silence and clipping rather than estimating from text', () => {
  const bytes = new Uint8Array(44 + 200); const view = new DataView(bytes.buffer);
  const text = (offset: number, value: string) => [...value].forEach((char, i) => bytes[offset + i] = char.charCodeAt(0));
  text(0, 'RIFF'); view.setUint32(4, bytes.length - 8, true); text(8, 'WAVEfmt '); view.setUint32(16, 16, true); view.setUint16(20, 1, true); view.setUint16(22, 1, true); view.setUint32(24, 100, true); view.setUint16(34, 16, true); text(36, 'data'); view.setUint32(40, 200, true);
  view.setInt16(44, 32767, true);
  expect(measurePcmWav(bytes)).toMatchObject({ durationSec: 1, silenceRatio: .99, clippedSamples: 1 });
