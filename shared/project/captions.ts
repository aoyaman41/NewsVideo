...
export const captionSchema = z.object({
  ...
  text: z.string(),
  timing: z.enum(['tts', 'estimated', 'manual']),
});
export type Caption = z.infer<typeof captionSchema>;
export const graphicSchema = z.object({
  enabled: z.boolean(),
  headline: z.string().max(100),
  keyNumber: z.string().max(40),
  source: z.string().max(180),
  bars: z.array(z.object({ label: z.string().max(30), value: z.number().min(0).max(100) })).max(4),
});
export function estimateCaptions(part: Part): Caption[] {
  ...
  const marks = new Map(
    part.audio?.timepoints?.map((mark) => [parseMarkIndex(mark.markName), mark.timeSeconds])
  );
  let previous = 0;
  return texts.map((text, index) => {
    const start = marks.get(index) ?? previous;
    const end =
      marks.get(index + 1) ?? Math.min(duration, start + (duration * text.length) / total);
    previous = end;
    return {
      ...
      timing: marks.has(index) ? 'tts' : 'estimated',
    };
  });
}
export function captionProblems(captions: Caption[], duration: number) {
  return captions.flatMap((cue, i) =>
    [
      cue.end <= cue.start ? `字幕${i + 1}: 終了が開始以前です` : '',
      cue.end > duration + 0.1 ? `字幕${i + 1}: 音声の尺を超えています` : '',
      i > 0 && cue.start < captions[i - 1].end ? `字幕${i + 1}: 前の字幕と重なっています` : '',
      cue.text.split('\n').some((line) => [...line].length > 30) || cue.text.split('\n').length > 2
        ? `字幕${i + 1}: 30文字×2行の目安を超えています`
        : '',
    ].filter(Boolean)
  );
}
