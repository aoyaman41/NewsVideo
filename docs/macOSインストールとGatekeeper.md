...
- これは未着手ではなく、個人開発 OSS の現状では Apple Developer Program の年額コストと運用負荷に見合わないため、現時点で意図的に見送っています
...
- 将来、配布規模や問い合わせ負荷に対してコストが見合うと判断したら、`Developer ID` 署名や `notarization` を再検討します
