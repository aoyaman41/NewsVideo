...
NewsVideo は、ニュース記事や解説記事からナレーション付き動画を作るための `macOS / Apple Silicon` 向けデスクトップアプリです。1つのプロジェクトの中で、記事入力、台本生成、画像生成、音声生成、動画書き出しまでを順番に進められます。

これは個人開発の `OSS Public Beta` です。配布版は self-serve 前提で、サポートは `best effort`、API 利用料はユーザー自身の契約に紐づきます。まずは `README`、`docs/クイックスタート.md`、`docs/FAQとトラブルシュート.md` を見ながら、自分で試せる人向けに公開しています。
...
- 画像スタイルを `図解 / エディトリアル / ミニマル / SNS カード / 文字入りスライド / データカード` から選ぶ
- 画像アスペクト比を `16:9 / 1:1 / 9:16` から選ぶ
- スライドサンプル画像を最大3枚まで参照し、画像生成のスタイルを揃える
- 音声の話し方を `ニュース調 / 落ち着いた解説 / カジュアル / プロモーション` から選ぶ
- 動画ごとに締めカードの見出し、CTA、出典表示を調整する
- パートごとに画像プロンプトを作り、画像生成まで進める
- ナレーション音声を生成する
- 共通の opening / ending 動画素材と、project ごとの締めカードを組み合わせて書き出す
...
5. 画像プロンプトと画像を生成する
6. 音声を生成する
7. 動画画面で締めカード、opening / ending の有無を確認して書き出す
...
- 既定の文章生成モデルは `GPT-5.2`、既定の画像生成と音声生成は Google 側の API を使います
- 文章生成に `GPT-6 Astra`、画像生成に `GPT Image 2` も選択できます。どちらも OpenAI APIキーと対象モデルの利用権限が必要です。既定の音声生成には引き続き Google AI APIキーを使います
- Astraの推論強度は `低 / 中 / 高 / 非常に高い / 最大`。GPT Image 2はOpenAI Images APIを使い、単体生成と一括生成に対応します
- GPT Image 2の横長・縦長Full HD相当は `1792×1008 / 1008×1792`、正方形4K相当は `2880×2880` です。これはAPIの画素数・16px単位制約に合わせたサイズで、2Kを超える画素数は実験的対応です
- 文章生成では `GPT-5.6 Sol / Terra / Luna` も選べます。品質優先なら `Sol`、品質と費用のバランスなら `Terra`、費用や大量処理を優先するなら `Luna` が目安です
- スクリプト生成モデルまたは画像プロンプト生成モデルのどちらかに OpenAI 系モデルを選ぶ場合は、`OpenAI` API キーが必要です
- GPT-5.6 系の推論強度は `なし / 低 / 中 / 高 / 非常に高い / 最大` です。`最小` は対象外で、用途ごとに異なる OpenAI 系モデルを選んだ場合は両方で使える値だけを選択できます
...
6. 動画画面で `締めカード`、`CTA`、`出典表示`、`opening / ending` の有無を確認して書き出す
...
- 動画画面の `締めカード / CTA / 出典表示` は project ごとに保存されます
- 設定画面の `opening / ending 動画` は app 全体の共通素材です
- 既存プロジェクトに `presentationProfile` がない場合は `news + preset closing` 互換で読み込みます
- `closing card` 系の設定がない既存 project は、互換性のため最初は `締めカードなし` で読み込みます
...
- 法令上許される最大限の範囲で、本ソフトウェアの利用または利用不能に起因する端末不具合、データ消失、情報漏えい、業務停止、逸失利益、外部サービス障害、API 利用料金負担、その他一切の損害について責任を負いません
...
用途プリセット、API不要の編集サンプル、原稿の即時保存、再開できる生成ジョブ、シーン編集、字幕と読み、出典照合、素材込みバックアップを追加しました。[操作ガイド](docs/制作ワークスペースガイド.md)を参照してください。
