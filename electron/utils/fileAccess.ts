...
const within = (root: string, candidate: string) => { const relative = path.relative(root, candidate); return relative === '' || (!relative.startsWith(`..${path.sep}`) && relative !== '..' && !path.isAbsolute(relative)); };
...
export class FileAccessPolicy {
  ...
  constructor(private roots: string[], private ledger: string) {}
  ...
  private async load() {
    ...
    try {
      ...
      this.grants = Array.isArray(parsed) ? parsed.filter((item): item is Grant => item && typeof item.path === 'string' && typeof item.write === 'boolean' && typeof item.directory === 'boolean') : [];
    } catch { this.grants = []; }
    return this.grants;
    })();
    return this.loading;
  ...
  async grant(file: string, write: boolean, directory = false) {
    ...
    if (existing) existing.write ||= write; else grants.push({ path: target, write, directory });
  ...
  async assert(file: unknown, write = false): Promise<string> {
    if (typeof file !== 'string' || !path.isAbsolute(file) || file.includes('\0')) throw new Error('絶対パスのファイルを指定してください。');
    ...
    throw new Error('このファイルへのアクセスは許可されていません。ファイル選択画面から指定してください。');
  }

  async media(file: unknown, write = false) {
    const target = await this.assert(file, write);
    if (!/\.(png|jpe?g|gif|webp|avif|mp4|mov|m4v|webm|wav|mp3|m4a|aiff?|flac|ogg)$/i.test(target)) throw new Error('許可されていないメディア形式です。');
    return target;
  }
}
...
export function fileAccess() {
  policy ??= new FileAccessPolicy([path.join(app.getPath('userData'), 'projects'), path.join(app.getPath('userData'), 'trash')], path.join(app.getPath('userData'), 'file-grants.json'));
  return policy;
}
