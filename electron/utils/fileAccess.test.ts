...
beforeEach(async () => {
  root = await fs.mkdtemp(path.join(os.tmpdir(), 'newsvideo-access-'));
  await fs.mkdir(path.join(root, 'projects'));
  policy = new FileAccessPolicy([path.join(root, 'projects')], path.join(root, 'grants.json'));
});
afterEach(async () => {
...
it('rejects traversal, sibling prefixes and symlink escapes', async () => {
  const secret = path.join(root, 'secret.enc');
  await fs.writeFile(secret, 'not a key');
  await fs.symlink(secret, path.join(root, 'projects', 'image.png'));
  await expect(policy.assert(path.join(root, 'projects', '..', 'secret.enc'))).rejects.toThrow(
    '許可'
  );
  await expect(policy.assert(path.join(root, 'projects-other', 'file.png'))).rejects.toThrow(
    '許可'
  );
...
it('persists exact read grants but never upgrades them to writes or whole directories', async () => {
  ...
  const restarted = new FileAccessPolicy(
  ...
  await expect(restarted.media(path.join(root, 'projects', 'project.json'))).rejects.toThrow(
    '形式'
  );
});
...
it('allows output creation only inside the selected directory', async () => {
  ...
  expect(await policy.media(path.join(output, 'final.mp4'), true)).toBe(
    path.join(await fs.realpath(output), 'final.mp4')
  );
