...
async function setup() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'nv-lifecycle-'));
  roots.push(root);
  const repo = new ProjectRepository(path.join(root, 'projects'));
  let project = await repo.create(createNewProject('sample', ''));
  const file = path.join(project.path, 'images', 'sample.png');
  await fs.writeFile(file, 'fixture');
  project = await repo.update(project.id, (data) => {
    data.article.bodyText = 'keep draft';
    data.article.importedImages.push({
...
it('rejects traversal and symlinked backup assets', async () => {
  ...
  data.project.article.importedImages[0].filePath = 'evil.png';
  await fs.writeFile(manifestPath, JSON.stringify(data));
