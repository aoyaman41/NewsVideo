...
function mapPaths<T>(value: T, transform: (file: string) => string): T {
  ...
  return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, (key === 'filePath' || key === 'lastVideoPath') && typeof item === 'string' ? transform(item) : mapPaths(item, transform)])) as T;
}
function allPaths(project: Project) { const result = new Set<string>(); mapPaths(project, (file) => { if (file) result.add(file); return file; }); return [...result]; }

export class ProjectLifecycle {
  constructor(private repo: ProjectRepository, private authorize: (file: string) => Promise<string>) {}
  async clone(id: string, template = false) {
    const source = await this.repo.load(id);
    if (source.job && ['running', 'queued'].includes(source.job.status)) throw new Error('生成を停止してから複製してください。');
    const target = createNewProject(`${source.name}${template ? ' テンプレート' : ' コピー'}`, '');
    const data = template ? { ...target, presentationProfile: { ...source.presentationProfile, styleReferenceImageIds: [] }, outputSettings: source.outputSettings, generationConfig: source.generationConfig, template: true } : { ...source, ...target, article: source.article, parts: source.parts, prompts: source.prompts, images: source.images, audio: source.audio, presentationProfile: source.presentationProfile, outputSettings: source.outputSettings, generationConfig: source.generationConfig };
    return this.copyIntoNew({ ...data, job: undefined, autoGenerationStatus: undefined, template }, (file) => this.authorize(file));
  }
  private async copyIntoNew(project: Project, resolve: (file: string) => Promise<string>) {
    ...
    try {
      ...
      const data = projectSchema.parse({ ...mapPaths(project, (file) => mapped.get(file) ?? file), path: directory, schemaVersion: 'v2.0', revision: 0, job: undefined, integrity: undefined, autoGenerationStatus: undefined, usage: [], archived: false });
  ...
  async export(id: string, destination: string) {
    ...
    if (project.job && ['running', 'queued'].includes(project.job.status)) throw new Error('生成を停止してからバックアップしてください。');
    // Job output logs may contain obsolete paths. The portable bundle contains the editable project and active assets.
    const portable = { ...project, job: undefined };
    ...
    try {
      ...
      await fs.writeFile(path.join(staging, 'backup.json'), JSON.stringify({ version: 1, project: { ...mapPaths(portable, (file) => mapped.get(file) ?? file), path: '.' } }), { mode: 0o600 });
      await fs.rename(staging, directory);
      return directory;
    } catch (error) { await fs.rm(staging, { recursive: true, force: true }); throw error; }
  }
  async import(directory: string) {
    ...
    return this.copyIntoNew({ ...project, id: randomUUID(), name: `${project.name} 復元`, createdAt: now, updatedAt: now }, async (file) => {
      if (path.isAbsolute(file) || file.split(/[\\/]/).includes('..')) throw new Error('バックアップに不正なパスがあります。');
      const resolved = await fs.realpath(path.join(root, file));
      const relative = path.relative(root, resolved);
      if (relative.startsWith('..') || path.isAbsolute(relative)) throw new Error('バックアップの範囲外を参照しています。');
      return resolved;
    });
  }
  async trash() {
    ...
    return (await fs.readdir(root, { withFileTypes: true })).filter((entry) => entry.isDirectory() && entry.name.endsWith('.newsproj')).map((entry) => ({ key: entry.name, name: entry.name }));
  }
  async restore(key: string) {
    ...
    try { await fs.access(target); throw new Error('同じIDのプロジェクトが存在します。'); } catch (error) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error; }
    await fs.rename(directory, target);
    const restored = await this.repo.load(project.id);
    return this.repo.save(mapPaths(restored, (file) => file.startsWith(`${original}${path.sep}`) ? path.join(target, path.relative(original, file)) : file));
  }
}
