...
export function registerOperation(name: string, handler: Parameters<typeof ipcMain.handle>[1]) {
  const execute: typeof handler = ['video:render', 'video:preview'].includes(name)
    ? (event, ...args) => {
  ...
  ipcMain.handle(name, (event, ...args) => {
    ...
    const devUrl =
      process.env.NEWSVIDEO_RENDERER_URL ||
      process.env.VITE_DEV_SERVER_URL ||
      'http://localhost:5173';
    if (
      !frame ||
      frame.parent ||
      !isTrustedRenderer(frame.url, app.isPackaged, app.getAppPath(), devUrl)
    )
