...
const DEFAULT_IMAGE_RESOLUTION = 'fhd';
const SUPPORTED_IMAGE_MODELS = new Set([
    'gemini-3.1-flash-image-preview',
    'gemini-3-pro-image-preview',
]);
const SUPPORTED_IMAGE_RESOLUTIONS = new Set(['fhd', '2k', '4k']);
// APIキーを読み込み
...
async function readImageGenerationSettings() {
    ...
    try {
        ...
        if (parsed.imageModel && SUPPORTED_IMAGE_MODELS.has(parsed.imageModel)) {
            imageModel = parsed.imageModel;
        }
        if (parsed.imageResolution &&
            SUPPORTED_IMAGE_RESOLUTIONS.has(parsed.imageResolution)) {
...
function getImageResolutionLabel(imageResolution) {
...
function buildImagePromptText(prompt, imageResolution) {
    ...
    const constraintsLine = `Aspect ratio: ${prompt.aspectRatio} (${getAspectRatioLabel(prompt.aspectRatio)}). Target resolution: ${dimensions.width}x${dimensions.height} (${getImageResolutionLabel(imageResolution)} preset). Keep this aspect ratio strictly.`;
...
// アスペクト比から寸法を計算
function getDimensions(aspectRatio, imageResolution) {
