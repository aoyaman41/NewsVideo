...
function sendProgress(payload) {
    const full = { source: 'video', ...payload };
...
async function readSettings() {
    try {
        ...
        return {
            ...
            videoPartLeadInSec: typeof parsed.videoPartLeadInSec === 'number' && Number.isFinite(parsed.videoPartLeadInSec)
...
async function resolveFfmpegPath() {
    ...
    if (!app.isPackaged) {
        try {
            ...
            if (ffmpegStaticPath && (await fileExists(ffmpegStaticPath)))
    ...
    throw new Error('ffmpegが見つかりません（アプリに同梱されていない可能性があります）。再インストールしてください。');
}
async function probeHasAudio(ffmpegPath, inputPath) {
...
async function runFfmpeg(ffmpegPath, args, job, onProgress) {
    ...
    await new Promise((resolve, reject) => {
        ...
        proc.on('close', (code, signal) => {
            ...
            if (code === 0) {
                ...
                reject(new Error(`ffmpegエラー (code=${code ?? 'null'}, signal=${signal ?? 'null'}): ${stderr.trim()}`));
            }
        });
    });
}
function buildImageMap(project) {
    const map = new Map();
    for (const img of project.images)
        map.set(img.id, img.filePath);
    for (const img of project.article.importedImages)
        map.set(img.id, img.filePath);
    return map;
}
function computeImageDurations(refs, totalDurationSec) {
    const n = refs.length;
    if (n === 0)
        return [];
...
async function writeImageConcatList(entries, listPath) {
    if (entries.length === 0)
...
async function normalizeVideoToSpec(ffmpegPath, inputPath, outputPath, options, job) {
    ...
    // 出力オプション（必ず全入力の後に置く）
    args.push('-vf', vf, '-r', String(options.fps), '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-preset', 'veryfast', '-b:v', options.videoBitrate, '-c:a', 'aac', '-b:a', options.audioBitrate, '-ar', '48000', '-ac', '2', '-movflags', '+faststart', outputPath);
    await runFfmpeg(ffmpegPath, args, job);
}
async function renderPartVideo(ffmpegPath, project, part, options, outputPath, job, leadInSec, onPercent) {
    ...
    const imagePaths = part.panelImages.map((ref) => {
        ...
        if (!p)
            throw new Error(`画像が見つかりません (imageId=${ref.imageId})`);
        return p;
    });
    const clampedLeadInSec = Number.isFinite(leadInSec) && leadInSec > 0 ? Math.min(2, Math.max(0, leadInSec)) : 0;
    ...
    const args = [
        ...
        '-ac',
        '2',
        ...(leadInMs > 0 ? ['-af', `adelay=${leadInMs}|${leadInMs}`] : []),
        '-shortest',
        '-movflags',
        '+faststart',
        outputPath,
...
async function concatSegments(ffmpegPath, segmentPaths, outputPath, options, job) {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'newsvideo-'));
...
async function requestVideoPathReauthorization(sourcePath, baseName) {
...
async function stageVideoInputForFfmpeg(sourcePath, tempDir, baseName) {
    ...
    try {
        ...
        const permissionError = nodeError?.code === 'EPERM' ||
        ...
        if (permissionError) {
            ...
            if (reselected) {
                try {
                    ...
                    throw new Error(`${baseName === 'opening' ? 'オープニング' : 'エンディング'}動画にアクセスできません: ${reselected}\n` +
                        `アクセス可能な場所へ移動して再指定してください。 (${retryDetail})`);
                }
            }
        }
        throw new Error(`${baseName === 'opening' ? 'オープニング' : 'エンディング'}動画にアクセスできません: ${sourcePath}\n` +
            `ファイル権限を確認するか、アクセス可能な場所へ移動して再指定してください。 (${detail})`);
    }
}
async function copyRenderedOutput(stagedOutputPath, outputPath) {
    try {
        await fs.copyFile(stagedOutputPath, outputPath);
    }
    catch (error) {
        const detail = formatUnknownError(error);
        throw new Error(`出力先に動画を書き込めません: ${outputPath}\n` +
            `出力先フォルダの権限を確認してください。 (${detail})`);
    }
}
async function findProjectByPartId(partId) {
...
ipcMain.handle('video:cancelRender', async () => {
    if (!currentJob)
        return { success: true };
...
ipcMain.handle('video:preview', async (_, partId) => {
    if (currentJob)
        throw new Error('別の動画処理が実行中です');
    const job = { canceled: false, processes: new Set() };
    currentJob = job;
    try {
        ...
        await renderPartVideo(ffmpegPath, project, part, previewOptions, previewPath, job, leadInSec, (within) => {
            sendProgress({
                stage: 'rendering_parts',
...
ipcMain.handle('video:render', async (_, project, options, outputPath) => {
    if (currentJob)
        throw new Error('別の動画処理が実行中です');
    const job = { canceled: false, processes: new Set() };
    currentJob = job;
    let renderTmpDir = null;
    try {
        if (!outputPath)
            throw new Error('出力先が未指定です');
        ...
        if (totalParts === 0)
            throw new Error('パートがありません');
        const generatedPartPaths = [];
        const partDurations = [];
        for (let i = 0; i < totalParts; i++) {
            ...
            const { durationSec } = await renderPartVideo(ffmpegPath, project, part, options, partOut, job, leadInSec, (within) => {
        ...
        if (options.includeOpening) {
            ...
            if (!opening)
                throw new Error('オープニング動画が未設定です（設定画面で指定してください）');
        ...
        if (options.includeEnding) {
            ...
            if (!ending)
                throw new Error('エンディング動画が未設定です（設定画面で指定してください）');
